...
def classifysingleSample(sample, decisionTree, details_of_path=[]):
    if not isinstance(decisionTree, dict):
        return decisionTree, details_of_path

    question = list(decisionTree.keys())[0]
    attribute, value = question.split(" <= ")
    details_of_path.append(question)
    ...
    return classifysingleSample(sample, answer, details_of_path)


def singlePredictions(dataFrame, decisionTree):
    predictions = dataFrame.apply(classifysingleSample, axis=1, args=(decisionTree,))
    return predictions
